...
public class AirSim : ModuleRules
{
    ...
    private void SetupCompileMode(CompileMode mode, ReadOnlyTargetRules Target)
    {
        ...
        LoadAirSimSharedDependency(Target, "armadillo", "armadillo");
        switch (mode)
        {
    ...
    private bool LoadAirSimSharedDependency(ReadOnlyTargetRules Target, string LibName, string LibFileName)
    {
        string LibrariesPath = Path.Combine(AirLibPath, "deps", LibName, "lib");
        return AddSharedLibDependency(LibName, LibrariesPath, LibFileName, Target, true);
    }


    private bool AddSharedLibDependency(string LibName, string LibPath, string LibFileName, ReadOnlyTargetRules Target, bool IsAddLibInclude)
    {
        ...
        if (Target.Platform == UnrealTargetPlatform.Win64)
        {
            ...
            isLibrarySupported = true;
            PublicAdditionalLibraries.Add(Path.Combine(LibPath, "lib" + LibFileName + ".so"));
        } else if (Target.Platform == UnrealTargetPlatform.Mac) {
            isLibrarySupported = true;
            PublicAdditionalLibraries.Add(Path.Combine(LibPath, "lib" + LibFileName + ".dylib"));
        }

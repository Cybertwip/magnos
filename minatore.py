...
def teleport_vacuum_plates(to_origin: bool, position: float = 0.5):
    ...
    if to_origin:
        ...
        print(f"Vacuum plates teleported to target planet at x-position: {position}")
...
def start_pygame_emergence_simulation(gravitational_potential):
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                running = False
            elif event.type == KEYDOWN and emerged:
                if event.key == K_LEFT:
                    gravitational_potential = teleport_vacuum_plates(False, position=(position_x - quantumite_size)/WINDOW_WIDTH)
                    auto_move_speed = -abs(gravitational_potential_to_speed(gravitational_potential))  # Ensure negative speed for left movement
                    teleport_count += 1  # Increment count
                    ...
                    auto_move_speed = abs(gravitational_potential_to_speed(gravitational_potential))  # Ensure positive speed for right movement
                    teleport_count += 1  # Increment count
                    # Right after incrementing teleport_count:
                    for i in range(len(plate_effects)):
                        plate_effects[i] += 0.01  # For example, increase effect by 0.1

                elif event.key == K_q:  # Check for Q key press
                    for x in range(0, teleport_count):
                        gravitational_potential = teleport_vacuum_plates(True)
                        auto_move_speed = gravitational_potential_to_speed(gravitational_potential)
                        # Right after incrementing teleport_count:
                        for i in range(len(plate_effects)):
                            plate_effects[i] -= 0.01  # For example, increase effect by 0.1
        ...
        position_x += auto_move_speed  # Auto-move the block to the right

        # Restrict Quantumite's horizontal movement within window bounds
        position_x = max(0, min(position_x, WINDOW_WIDTH - quantumite_size))
        ...
        if position_y + quantumite_size > WINDOW_HEIGHT // 2:
            position_y -= 1
        else:
            if not emerged:
                gravitational_potential = teleport_vacuum_plates(True)

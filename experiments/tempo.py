...
# Updated Lopez-Ahlgrimm equation terms
...
# Extract counts and states from the result
states, counts = zip(*counts.items())

# Create a simple index for the bar chart
index = range(len(states))

# Plot the bar chart with a simple index
ax1.barh(states, counts, color='blue')
ax1.set_xlabel('Counts')
ax1.set_ylabel('States')
ax1.set_title('Quantum Histogram')

# Plot the energy levels
ax2.bar(["Total Energy", "Measured Energy"], [total_energy, measured_energy], color=["blue", "orange"])
ax2.set_ylabel("Value")
ax2.set_title("Total Energy and Measured Energy - " + state_label)

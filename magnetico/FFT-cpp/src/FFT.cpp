...
// Function to compute pow(w_k, num_pairs * i.real()) for complex numbers
std::complex<double> customPow(const std::complex<double>& base, double exponent) {
	return std::polar(1.0, exponent * std::arg(base));
}

// Function to apply the customPow element-wise to Eigen vectors
Eigen::VectorXcd applyCustomPow(const Eigen::VectorXcd& base, double exponent) {
	Eigen::VectorXcd result = base.unaryExpr([&](const std::complex<double>& w_k) {
		return customPow(w_k, exponent);
	});
	return result;
}
...
Eigen::VectorXcd computation(Eigen::VectorXcd&& x_seg_A, Eigen::VectorXcd&& x_seg_B, int N)
{
    ...
    */
	...
	Eigen::VectorXcd indices(W_K.size());
	for (int i = 0; i < W_K.size(); ++i) {
		indices(i) = std::complex<double>(i, 0);  // Real part is the index, imaginary part is 0
	}

	// Apply the custom power function element-wise using unaryExpr
	W_K = W_K.unaryExpr([&](const std::complex<double>& w_k) {
		int index = static_cast<int>(w_k.real());
		return indices(index) * w_k;
	});

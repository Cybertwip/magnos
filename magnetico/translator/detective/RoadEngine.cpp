...
RoadEngine::RoadEngine(int channels) : OnnxEngine(std::string{MODELS_PATH} + "hybridnets/384x512/roadseg.onnx") {
...
std::vector<float> RoadEngine::prepareInputImage(Image& image, const std::vector<float>& mean, const std::vector<float>& std) {
...
std::tuple<Image> RoadEngine::detectLanes(Image& image){
...
std::tuple<std::vector<std::vector<std::size_t>>> RoadEngine::process_output(const Ort::Value& tensor, const Config& cfg) {
